...
from collections import Counterheapq
...
to_visit: list[tuple[int, int, int, str, deque[tuple[int, int]]]] = [
    (0, *start, ">", deque())
]
best_path_tiles: set[tuple[int, int]] = set()

min_cost = float("inf")
while to_visit:
    cost, x, y, dir, path = heapq.heappop(to_visit)
    ...
    for ndir, (dx, dy) in DIRECTIONS.items():
        ...
        if (nx, ny, ndir) not in visited or visited[(nx, ny, ndir)] >= ncost:
            heapq.heappush(to_visit, (ncost, nx, ny, ndir, deque(path)))
